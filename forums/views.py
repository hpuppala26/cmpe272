...
def topic_create_view(request, slug):
	if request.user.is_authenticated:
		myform = CreateTopicForm()
		if request.method == "POST":
			myform = CreateTopicForm(request.POST)
			if myform.is_valid():
				finaldata = myform.cleaned_data
				finaldata['creatorid'] = request.user
				finaldata['forumid'] = Forum.objects.get(slug=slug)
				Topic.objects.create(**finaldata)
				return redirect('forumpage',slug)
			else:
				print(myform.errors)
		context = {"form" : myform,}
